...
এটা$n$টি সেটের সংযোগের জন্যও সাধারণীকরণ করা যায়।
...
আমরা প্রুফ করতে চাই যে অন্তত একটি সেট $A_i$-তে থাকা যেকোনো উপাদান সূত্রে শুধুমাত্র একবার গণনা করা হবে (লক্ষ্য করো যে কোনো $A_i$ সেটে নেই এমন উপাদানগুলো সূত্রের ডান পাশে কখনোই কনসিডার করা হবে না)।
...
এটাআমাদের [বাইনোমিয়াল কোএফিশিয়েন্ট](binomial-coefficients.md)-এর নিচের যোগফলে নিয়ে যায়:

$$ T = \binom{k}{1} - \binom{k}{2} + \binom{k}{3} - \cdots + (-1)^{i-1}\cdot \binom{k}{i} + \cdots + (-1)^{k-1}\cdot \binom{k}{k}$$

এই রাশিটি $(1 - x)^k$-এর বাইনোমিয়াল বিস্তারের সাথে খুব সাদৃশ্যপূর্ণ:

$$ (1 - x)^k = \binom{k}{0} - \binom{k}{1} \cdot x + \binom{k}{2} \cdot x^2 - \binom{k}{3} \cdot x^3 + \cdots + (-1)^k\cdot \binom{k}{k} \cdot x^k $$

যখন $x = 1$, $(1 - x)^k$ দেখতে $T$-এর মতো। তবে রাশিটিতে একটি অতিরিক্ত $\binom{k}{0} = 1$ আছে এবং এটা$-1$ দ্বারা গুণিত। এতে পাওয়া যায় $(1 - 1)^k = 1 - T$। অতএব $T = 1 - (1 - 1)^k = 1$, যা প্রুফ করতে হয়েছিল। উপাদানটি শুধুমাত্র একবার গণনা করা হয়।
...
এর সাধারণীকরণ কনসিডার করো ঠিক $r$টি সেটে থাকা উপাদানের সংখ্যা গণনা করতে:
...
এই সূত্রটি প্রুফ করতে, একটি নির্দিষ্ট $B$ কনসিডার করো। মৌলিক ইনক্লুশন-এক্সক্লুশন নীতি অনুসারে এটাসম্পর্কে বলা যায়:
...
বাম পাশের সেটগুলো বিভিন্ন $B$-এর জন্য ছেদ করে না, তাই আমরা সরাসরি যোগ করতে পারি। এছাড়াও লক্ষ্য করো যে যেকোনো সেট $X$ সবসময় $(-1)^{m-r}$ সহগ পাবে যদি এটাউপস্থিত হয় এবং এটাঠিক $\dbinom{m}{r}$টি সেট $B$-এর জন্য উপস্থিত হবে।
...
$x_i$-এর সীমাবদ্ধতা মুহূর্তের জন্য ভুলে যাও এবং শুধু এই ইকুয়েশনের অ-ঋণাত্মক সমাধানের সংখ্যা গোনো। এটা[স্টার্স অ্যান্ড বার্স](stars_and_bars.md) ব্যবহার করে সহজেই করা যায়:
...
এটাসহজেই সাধারণীকরণ করা যায় $d$টি সংখ্যার জন্য যাদের যোগফল $s$ এবং সীমাবদ্ধতা $0 \le x_i \le b$:

$$\sum_{i=0}^d (-1)^i \binom{d}{i} \binom{s+d-1-(b+1)i}{d-1}$$

উপরের মতো, ঋণাত্মক উপরের ইনডেক্স বিশিষ্ট বাইনোমিয়াল কোএফিশিয়েন্টকে শূন্য হিসেবে গণ্য করি।
...
তবে, আমরা যদি এই সংখ্যাগুলো সরাসরি যোগ করি, কিছু সংখ্যা একাধিকবার গণনা করা হবে (যেগুলোর একাধিক $p_i$ ফ্যাক্টর আছে)। তাই ইনক্লুশন-এক্সক্লুশন নীতি ব্যবহার করা প্রয়োজন।
...
সমাধান অ্যালগরিদম আগের সমস্যার সাথে প্রায় অভিন্ন — $a_i$ সংখ্যাগুলোর উপর ইনক্লুশন-এক্সক্লুশনের সূত্র তৈরি করো, অর্থাৎ এই সূত্রের প্রতিটি পদ হলো $a_i$ সংখ্যাগুলোর একটা দেওয়া সাবসেট দ্বারা বিভাজ্য সংখ্যার পরিমাণ (অন্যভাবে বললে, তাদের [লসাগু](../algebra/euclid-algorithm.md) দ্বারা বিভাজ্য)।
...
তবে, এই সমাধানের কমপ্লেক্সিটি $O(3^k \cdot k)$। উন্নত করতে, লক্ষ্য করো বিভিন্ন $ans(X)$ গণনায় প্রায়ই $Y$ সেট শেয়ার হয়।
...
Graham-এর (Graham, Knuth, Patashnik. "Concrete mathematics" [1998]) দিকে তাকালে, [বাইনোমিয়াল কোএফিশিয়েন্ট](binomial-coefficients.md)-এর একটি সুপরিচিত সূত্র দেখি:

$$ \sum_{k=0}^m (-1)^k \cdot \binom{n}{k} = (-1)^m \cdot \binom{n-1}{m} $$

এটাএখানে প্রয়োগ করলে, বাইনোমিয়াল কোএফিশিয়েন্টের পুরো যোগফল সংক্ষিপ্ত হয়:
...
এছাড়াও বাধা ছাড়া সমস্যা সমাধান করতে শেখো: অর্থাৎ এক ঘর থেকে অন্য ঘরে যাওয়ার উপায়ের সংখ্যা গুনতে শেখো। একটি অক্ষে $x$ ঘর এবং অন্যটিতে $y$ ঘর যেতে হবে। সরল কম্বিনেটরিক্স থেকে, [বাইনোমিয়াল কোএফিশিয়েন্ট](binomial-coefficients.md) ব্যবহার করে সূত্র পাই:

$$\binom{x+y}{x}$$

এখন সব বাধা এড়িয়ে এক ঘর থেকে অন্য ঘরে যাওয়ার উপায়ের সংখ্যা গুনতে, ইনক্লুশন-এক্সক্লুশন ব্যবহার করে বিপরীত সমস্যা সমাধান করা যায়: বাধার একটি সাবসেটের উপর দিয়ে হেঁটে যাওয়ার উপায়ের সংখ্যা গোনো (এবং মোট উপায়ের সংখ্যা থেকে বাদ দাও)।
...
যেখানে $deg(d)$ হলো $d$ সংখ্যার ফ্যাক্টররণে মৌলিক সংখ্যার পরিমাণ এবং $f(d)$ হলো $d$ দ্বারা বিভাজ্য চতুষ্টয়ের সংখ্যা।

$f(d)$ ফাংশন গণনা করতে, শুধু $d$-এর গুণিতকের সংখ্যা গুনতে হবে (যেমন আগের সমস্যায় বলা হয়েছে) এবং [বাইনোমিয়াল কোএফিশিয়েন্ট](binomial-coefficients.md) ব্যবহার করে তাদের মধ্য থেকে চারটি বাছাই করার উপায়ের সংখ্যা গুনতে হবে।
...
1. প্রথমে, $[2;n]$ ব্যবধানে এমন সব সংখ্যা খুঁজুন যাদের সরল ফ্যাক্টররণে কোনো মৌলিক ফ্যাক্টর দুইবার নেই। এই সংখ্যাগুলোর জন্য কতগুলো ফ্যাক্টর আছে তাও জানতে হবে।
...
(যদি এই রাশিটি নিকটতম পূর্ণসংখ্যায় রাউন্ড করো — তুমি ঠিক স্থির বিন্দু ছাড়া পারমুটেশনের সংখ্যা পাবেন)
