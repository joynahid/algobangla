...
সমতলে বিন্দুর একটা সেট $\{p_i\}$ ধরো।
...
একটি বিরক্তিকর অবক্ষয়িত ক্ষেত্র আছে যখন ভরোনয় ডায়াগ্রাম কানেক্টেড (connected) নয় এবং ডেলোনে ট্রায়াঙ্গুলেশন বিদ্যমান নেই। এই ক্ষেত্রটি হলো যখন সব বিন্দু সমরেখ।
...
ধরো $\{p_i\}$ কলিনিয়ার না আর $\{p_i\}$-র মধ্যে কোনো চারটা বিন্দু একটা বৃত্তে নেই। তাহলে $V(\{p_i\})$ আর $D(\{p_i\})$ ডুয়াল (dual), তাই যদি আমরা একটা পাই, অন্যটা $O(n)$-এ পেতে পারি। এমন না হলে কী করতে হবে? কলিনিয়ার কেস সহজেই প্রসেস করা যায়। অন্যথায়, $V$ আর $D'$ ডুয়াল, যেখানে $D'$ হলো $D$ থেকে সেই সব এজ সরিয়ে পাওয়া যায় যেখানে এজের দুই পাশের ত্রিভুজ সার্কামসার্কেল (circumcircle) শেয়ার করে।

## ডেলোনে ও ভরোনয় নির্মাণ

ডুয়ালিটির কারণে, আমাদের শুধু $V$ আর $D$-র একটা দ্রুত বের করার অ্যালগরিদম দরকার। আমরা দেখাব কীভাবে $D(\{p_i\})$ $O(n\log n)$-এ তৈরি করা যায়। ট্রায়াঙ্গুলেশন Guibas আর Stolfi-র ডিভাইড অ্যান্ড কনকার (divide and conquer) অ্যালগরিদম দিয়ে তৈরি হবে।

## কোয়াড-এজ ডেটা স্ট্রাকচার

অ্যালগরিদম চলাকালীন $D$ কোয়াড-এজ ডাটা স্ট্রাকচারে স্টোর করা থাকবে। এই স্ট্রাকচার চিত্রে দেখানো হয়েছে:
<div style="text-align: center;">
  <img src="/images/geometry/quad-edge.png" alt="Quad-Edge">
</div>

অ্যালগরিদমে আমরা এজের উপর নিচের ফাংশনগুলো ব্যবহার করব:

  ১. `make_edge(a, b)`<br>
    এই ফাংশনটা বিন্দু `a` থেকে বিন্দু `b`-তে একটা বিচ্ছিন্ন এজ তৈরি করে, এর বিপরীত এজ আর উভয় ডুয়াল এজসহ।
  ২. `splice(a, b)`<br>
    এটি অ্যালগরিদমের একটি মূল ফাংশন। এটি `a->Onext`-কে `b->Onext` এবং `a->Onext->Rot->Onext`-কে `b->Onext->Rot->Onext`-এর সাথে অদলবদল করে।
  ৩. `delete_edge(e)`<br>
    এই ফাংশনটা ট্রায়াঙ্গুলেশন থেকে e ডিলিট করে। `e` ডিলিট করতে, আমরা শুধু `splice(e, e->Oprev)` আর `splice(e->Rev, e->Rev->Oprev)` কল করি।
  ৪. `connect(a, b)`<br>
    এই ফাংশনটা `a->Dest` থেকে `b->Org`-তে একটা নতুন এজ `e` তৈরি করে এমনভাবে যে `a`, `b`, `e` সবার একই বাম ফেস থাকে। এটা করতে, আমরা `e = make_edge(a->Dest, b->Org)`, `splice(e, a->Lnext)` আর `splice(e->Rev, b)` কল করি।
...
চলো সব বিন্দু x অনুসারে সাজাই, আর যদি $x_1 = x_2$ হয় তাহলে y অনুসারে। চলো কোনো সেগমেন্ট $(l, r)$-র জন্য সমস্যা সমাধান করি (শুরুতে $(l, r) = (0, n - 1)$)। যদি $r - l + 1 = 2$ হয়, আমরা একটি এজ $(p[l], p[r])$ যোগ করব এবং রিটার্ন করব। যদি $r - l + 1 = 3$ হয়, আমরা প্রথমে এজ $(p[l], p[l + 1])$ এবং $(p[l + 1], p[r])$ যোগ করব। এগুলো `splice(a->Rev, b)` ব্যবহার করে কানেক্ট করতে হবে। এখন আমাদের ত্রিভুজ বন্ধ করতে হবে। পরবর্তী ক্রিয়া $p[l], p[l + 1], p[r]$-র অরিয়েন্টেশনের উপর নির্ভর করবে। সমরেখ হলে ত্রিভুজ তৈরি করা যায় না, তাই কেবল `(a, b->Rev)` রিটার্ন করি। অন্যথায়, `connect(b, a)` কল করে একটি নতুন এজ `c` তৈরি করি। বিন্দুগুলো ঘড়ির কাঁটার বিপরীতে থাকলে, `(a, b->Rev)` রিটার্ন করি। অন্যথায় `(c->Rev, c)` রিটার্ন করি।

এখন ধরি $r - l + 1 \ge 4$। প্রথমে, চলো $L = (l, \frac{l + r}{2})$ আর $R = (\frac{l + r}{2} + 1, r)$ রিকার্সিভভাবে সমাধান করি। এখন আমাদের এই ট্রায়াঙ্গুলেশনগুলো একটিতে মার্জ করতে হবে। লক্ষ্য করো আমাদের বিন্দুগুলো সাজানো, তাই মার্জ করার সময় আমরা L থেকে R-তে এজ যোগ করব (তথাকথিত _ক্রস_ এজ) এবং L থেকে L এবং R থেকে R-তে কিছু এজ সরাব।
ক্রস এজের গঠন কেমন? এই সব এজকে y-অক্ষের সমান্তরাল একটি রেখা অতিক্রম করতে হবে যা স্প্লিটিং x মানে স্থাপিত। এটি ক্রস এজগুলোর একটি লিনিয়ার ক্রম স্থাপন করে, তাই আমরা পরপর ক্রস এজ, সবচেয়ে নিচের ক্রস এজ ইত্যাদি নিয়ে কথা বলতে পারি। অ্যালগরিদম ক্রস এজগুলো ক্রমবর্ধমান ক্রমে যোগ করবে। লক্ষ্য করো, যেকোনো দুটো সংলগ্ন ক্রস এজের একটা সাধারণ প্রান্তবিন্দু থাকবে, আর তাদের ডিফাইন করা ত্রিভুজের তৃতীয় বাহু L থেকে L-তে বা R থেকে R-তে যায়। চলো বর্তমান ক্রস এজকে বেস বলি। বেসের পরবর্তী হবে হয় বেসের বাম প্রান্তবিন্দু থেকে ডান প্রান্তবিন্দুর R-প্রতিবেশীদের একটিতে অথবা বিপরীতভাবে।
বেস আর আগের ক্রস এজের সার্কামসার্কেল ধরো।
ধরো এই বৃত্তটা অন্য বৃত্তে রূপান্তরিত হচ্ছে যার বেস একটা জ্যা কিন্তু Oy দিকে আরো দূরে থাকে।
...
এটা এফিশিয়েন্টলি করতে, আমরা দুটো এজ `lcand` আর `rcand` হিসাব করি যেন `lcand` এই প্রসেসে পাওয়া প্রথম L বিন্দুতে পয়েন্ট করে, আর `rcand` প্রথম R বিন্দুতে পয়েন্ট করে।
তারপর আমরা সেটা বেছে নিই যেটা আগে পাওয়া যাবে। শুরুতে বেস L আর R-এর নিচের ট্যানজেন্টে পয়েন্ট করে।

## ইমপ্লিমেন্টেশন

লক্ষ্য করো, in_circle ফাংশনের ইমপ্লিমেন্টেশন GCC-স্পেসিফিক।
