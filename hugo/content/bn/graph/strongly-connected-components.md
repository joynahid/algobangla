---
title: "স্ট্রংলি কানেক্টেড কম্পোনেন্ট এবং কনডেনসেশন গ্রাফ"
tags: 
weight: 50
---
...
## সংজ্ঞাসমূহ
ধরি $G=(V,E)$ একটি ডিরেক্টেড গ্রাফ যার ভার্টেক্স $V$ এবং এজ $E \subseteq V \times V$। আমরা $n=|V|$ দিয়ে ভার্টেক্স সংখ্যা এবং $m=|E|$ দিয়ে $G$-তে এজ সংখ্যা বোঝাই। এই আর্টিকেলের সব সংজ্ঞা মাল্টিগ্রাফে সম্প্রসারণ করা সহজ, তবে আমরা সেদিকে মনোযোগ দেব না।

$C \subseteq V$ ভার্টেক্সের একটি উপসেটকে **স্ট্রংলি কানেক্টেড কম্পোনেন্ট** বলা হয় যদি নিচের শর্তগুলো পূরণ হয়:
...
এই গ্রাফ $G_\text{example}$ কনসিডার করো, যেখানে স্ট্রংলি কানেক্টেড কম্পোনেন্টগুলো হাইলাইট করা হয়েছে:
...
আমরা **কনডেনসেশন গ্রাফ** $G^{\text{SCC}}=(V^{\text{SCC}}, E^{\text{SCC}})$ এভাবে ডিফাইন করি:
...
$G_\text{example}$-এর কনডেনসেশন গ্রাফ এরকম:

<center><img src="/images/graph/cond_graph.svg" alt="drawing" style="width:600px;"/></center>


কনডেনসেশন গ্রাফের সবচেয়ে গুরুত্বপূর্ণ বৈশিষ্ট্য হলো এটি **অ্যাসাইক্লিক**। আসলে, সংজ্ঞা অনুযায়ী কনডেনসেশন গ্রাফে কোনো 'সেল্ফ-লুপ' নেই, এবং যদি দুই বা ততোধিক ভার্টেক্সের (স্ট্রংলি কানেক্টেড কম্পোনেন্ট) মধ্য দিয়ে কোনো সাইকেল থাকত, তাহলে পৌঁছানোযোগ্যতার কারণে এই স্ট্রংলি কানেক্টেড কম্পোনেন্টগুলোর ইউনিয়ন নিজেই একটি স্ট্রংলি কানেক্টেড কম্পোনেন্ট হতো: বিরোধ।

পরবর্তী বিভাগে দেখানো অ্যালগরিদম একটা দেওয়া গ্রাফে সব স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে বের করে। এরপর, কনডেনসেশন গ্রাফ তৈরি করা যায়।

## কোসারাজুর অ্যালগরিদম

### অ্যালগরিদমের বর্ণনা
দেখানো অ্যালগরিদমটি স্বাধীনভাবে কোসারাজু এবং শরীর ১৯৮০ সালের দিকে প্রস্তাব করেছিলেন। এটি দুটি সিরিজের [DFS](depth-first-search.md)-এর উপর ভিত্তি করে, রানটাইম $O(n + m)$।

অ্যালগরিদমের প্রথম ধাপে, আমরা সম্পূর্ণ গ্রাফ ভিজিট করে DFS-এর একটি ক্রম সম্পাদন করি (`dfs`)। অর্থাৎ, যতক্ষণ অভিজিত ভার্টেক্স আছে, আমরা তাদের একটি নিই এবং সেই ভার্টেক্স থেকে DFS শুরু করি। প্রতিটি ভার্টেক্সের জন্য, আমরা *এক্সিট টাইম* $t_\text{out}[v]$ ট্র্যাক রাখি। এটি সেই 'টাইমস্ট্যাম্প' যখন ভার্টেক্স $v$-তে `dfs`-এর এক্সিকিউশন শেষ হয়, অর্থাৎ $v$ থেকে পৌঁছানো সব ভার্টেক্স ভিজিট হয়ে গেছে এবং অ্যালগরিদম $v$-তে ফিরে এসেছে। টাইমস্ট্যাম্প কাউন্টার পরপর `dfs` কলের মধ্যে *রিসেট হওয়া উচিত নয়*। এক্সিট টাইম অ্যালগরিদমে গুরুত্বপূর্ণ ভূমিকা পালন করে, যা নিচের থিওরেম আলোচনা করলে স্পষ্ট হবে।

প্রথমে, আমরা একটি স্ট্রংলি কানেক্টেড কম্পোনেন্ট $C$-এর এক্সিট টাইম $t_\text{out}[C]$ সংজ্ঞায়িত করি $C$-এর সব $v$-এর $t_\text{out}[v]$ মানের সর্বোচ্চ হিসেবে। এছাড়াও, থিওরেমের প্রুফে, আমরা প্রতিটি ভার্টেক্স $v\in G$-এর *এন্ট্রি টাইম* $t_{\text{in}}[v]$ উল্লেখ করব। $t_{\text{in}}[v]$ সংখ্যাটি সেই 'টাইমস্ট্যাম্প' যখন অ্যালগরিদমের প্রথম ধাপে ভার্টেক্স $v$-তে রিকার্সিভ ফাংশন `dfs` কল করা হয়। একটি স্ট্রংলি কানেক্টেড কম্পোনেন্ট $C$-এর জন্য, আমরা $t_{\text{in}}[C]$ সংজ্ঞায়িত করি $C$-এর সব $v$-এর $t_{\text{in}}[v]$ মানের সর্বনিম্ন হিসেবে।
...
দুটি ভিন্ন ক্ষেত্র আছে, কোন কম্পোনেন্ট DFS-এ আগে পৌঁছানো হবে তার উপর নির্ভর করে:

- ক্ষেত্র ১: কম্পোনেন্ট $C$ আগে পৌঁছানো হয়েছে (অর্থাৎ, $t_{\text{in}}[C] < t_{\text{in}}[C']$)। এক্ষেত্রে, DFS কোনো সময়ে $C$-এর কোনো ভার্টেক্স $v$ ভিজিট করে যখন $C$ এবং $C'$ কম্পোনেন্টের অন্য সব ভার্টেক্স এখনো অভিজিত। যেহেতু কনডেনসেশন গ্রাফে $C$ থেকে $C'$-তে এজ আছে, তাই $C$-র অন্য সব ভার্টেক্সের পাশাপাশি $C'$-এর সব ভার্টেক্সও $v$ থেকে পৌঁছানো যায়। এর মানে $v$ থেকে চলমান এই `dfs` এক্সিকিউশন ভবিষ্যতে $C$ এবং $C'$ কম্পোনেন্টের অন্য সব ভার্টেক্সও ভিজিট করবে, তাই এই ভার্টেক্সগুলো DFS ট্রি-তে $v$-এর ডিসেন্ডেন্ট হবে। এ থেকে বোঝা যায় যে প্রতিটি ভার্টেক্স $u \in (C \cup C')\setminus \{v\}$-এর জন্য, $t_\text{out}[v] > t_\text{out}[u]$। সুতরাং, $t_\text{out}[C] > t_\text{out}[C']$, যা এই ক্ষেত্রের প্রুফ সম্পূর্ণ করে।

- ক্ষেত্র ২: কম্পোনেন্ট $C'$ আগে পৌঁছানো হয়েছে (অর্থাৎ, $t_{\text{in}}[C] > t_{\text{in}}[C']$)। এক্ষেত্রে, DFS কোনো সময়ে $C'$-এর কোনো ভার্টেক্স $v$ ভিজিট করে যখন $C$ এবং $C'$ কম্পোনেন্টের অন্য সব ভার্টেক্স এখনো অভিজিত। যেহেতু কনডেনসেশন গ্রাফে $C$ থেকে $C'$-তে এজ আছে, অ্যাসাইক্লিসিটি বৈশিষ্ট্য অনুযায়ী $C'$ থেকে $C$ পৌঁছানো যায় না। তাই $v$ থেকে চলমান `dfs` এক্সিকিউশন $C$-এর কোনো ভার্টেক্সে পৌঁছাবে না, কিন্তু এটি $C'$-এর সব ভার্টেক্স ভিজিট করবে। $C$-এর ভার্টেক্সগুলো এই ধাপে পরে কোনো `dfs` এক্সিকিউশনে ভিজিট হবে, তাই আসলে $t_\text{out}[C] > t_\text{out}[C']$। এটি প্রুফ সম্পূর্ণ করে।
{{< /details >}}
প্রুফিত থিওরেমটি স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে বের করার জন্য অত্যন্ত গুরুত্বপূর্ণ। এর অর্থ হলো কনডেনসেশন গ্রাফের যেকোনো এজ বেশি $t_\text{out}$ মানের কম্পোনেন্ট থেকে কম $t_\text{out}$ মানের কম্পোনেন্টে যায়।

যদি আমরা সব ভার্টেক্স $v \in V$-কে তাদের এক্সিট টাইম $t_\text{out}[v]$-এর নিম্নক্রমে সাজাই, তাহলে প্রথম ভার্টেক্স $u$ "রুট" স্ট্রংলি কানেক্টেড কম্পোনেন্টে থাকবে, যার কনডেনসেশন গ্রাফে কোনো ইনকামিং এজ নেই। এখন আমরা এই ভার্টেক্স $u$ থেকে এমন কোনো সার্চ চালাতে চাই যেটি তার স্ট্রংলি কানেক্টেড কম্পোনেন্টের সব ভার্টেক্স ভিজিট করবে, কিন্তু অন্য ভার্টেক্স নয়। বারবার এটি করে, আমরা ধীরে ধীরে সব স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে পেতে পারি: প্রথম পাওয়া কম্পোনেন্টের সব ভার্টেক্স সরিয়ে, $t_\text{out}$-এর সবচেয়ে বড় মানের পরবর্তী অবশিষ্ট ভার্টেক্স খুঁজে সেখান থেকে এই সার্চ চালাই, এভাবে চলতে থাকবে। শেষে, আমরা সব স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে পাব। আমাদের ইচ্ছামতো আচরণ করে এমন একটি সার্চ পদ্ধতি খুঁজতে, নিচের থিওরেম কনসিডার করো:
...
{{< /hint >}}
প্রুফ বাদ দেওয়া হলো (তবে সরলরেখীয়)। এই থিওরেমের তাহলে, $G^T$-এর কনডেনসেশন গ্রাফে "রুট" কম্পোনেন্ট থেকে অন্য কম্পোনেন্টে কোনো এজ থাকবে না। সুতরাং, ভার্টেক্স $v$ ধারণকারী পুরো "রুট" স্ট্রংলি কানেক্টেড কম্পোনেন্ট ভিজিট করতে, আমরা ট্রান্সপোজ গ্রাফ $G^T$-এ ভার্টেক্স $v$ থেকে একটি DFS চালাতে পারি! এটি ঠিক এই স্ট্রংলি কানেক্টেড কম্পোনেন্টের সব ভার্টেক্স ভিজিট করবে। পূর্বে উল্লেখ করা হয়েছে, আমরা তারপর এই ভার্টেক্সগুলো গ্রাফ থেকে সরাতে পারি। তারপর, $t_\text{out}[v]$-এর সর্বোচ্চ মানের পরবর্তী ভার্টেক্স খুঁজে সেই ভার্টেক্স থেকে ট্রান্সপোজ গ্রাফে সার্চ চালিয়ে পরবর্তী স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে বের করি। পুনরাবৃত্তি করে, আমরা সব স্ট্রংলি কানেক্টেড কম্পোনেন্ট পাই।

সুতরাং, সংক্ষেপে, আমরা স্ট্রংলি কানেক্টেড কম্পোনেন্ট খুঁজে বের করার নিচের অ্যালগরিদম আলোচনা করলাম:

 - ধাপ ১। $G$-তে DFS-এর একটি ক্রম চালান, যেটি ভার্টেক্সের একটি তালিকা (যেমন `order`) দেবে, ক্রমবর্ধমান এক্সিট টাইম $t_\text{out}$ অনুসারে সাজানো।

- ধাপ ২। ট্রান্সপোজ গ্রাফ $G^T$ তৈরি করো, এবং বিপরীত ক্রমে (অর্থাৎ এক্সিট টাইমের নিম্নক্রমে) ভার্টেক্সগুলোতে DFS-এর একটি সিরিজ চালান। প্রতিটি DFS একটি স্ট্রংলি কানেক্টেড কম্পোনেন্ট দেবে।

- ধাপ ৩ (ঐচ্ছিক)। কনডেনসেশন গ্রাফ তৈরি করো।

অ্যালগরিদমের রানটাইম কমপ্লেক্সিটি $O(n + m)$, কারণ DFS দুইবার করা হয়। কনডেনসেশন গ্রাফ তৈরিও $O(n+m)$।
...
`dfs` ফাংশনটি DFS ইমপ্লিমেন্ট করে। এটি ইনপুট হিসেবে একটি অ্যাডজেসেন্সি লিস্ট এবং একটি স্টার্টিং ভার্টেক্স নেয়। এটি `output` ভেক্টরের একটি রেফারেন্সও নেয়: প্রতিটি ভিজিট করা ভার্টেক্স `dfs` সেটি ছেড়ে যাওয়ার সময় `output`-এ যোগ হবে।

লক্ষ্য কর যে আমরা `dfs` ফাংশন অ্যালগরিদমের প্রথম এবং দ্বিতীয় উভয় ধাপেই ব্যবহার করি। প্রথম ধাপে, আমরা $G$-এর অ্যাডজেসেন্সি লিস্ট পাস করি, এবং পরপর `dfs` কলে আমরা একই 'আউটপুট ভেক্টর' `order` পাস করতে থাকি, যেন শেষ পর্যন্ত আমরা ক্রমবর্ধমান এক্সিট টাইমের ক্রমে ভার্টেক্সের তালিকা পাই। দ্বিতীয় ধাপে, আমরা $G^T$-এর অ্যাডজেসেন্সি লিস্ট পাস করি, এবং প্রতিটি কলে একটি খালি 'আউটপুট ভেক্টর' `component` পাস করি, যেটি আমাদের একটি করে স্ট্রংলি কানেক্টেড কম্পোনেন্ট দেবে।

## টারজানের স্ট্রংলি কানেক্টেড কম্পোনেন্ট অ্যালগরিদম

### অ্যালগরিদমের বর্ণনা

দেখানো অ্যালগরিদমটি প্রথম ১৯৭২ সালে টারজান প্রস্তাব করেছিলেন।
এটি DFS কলের একটি ক্রম সম্পাদনের উপর ভিত্তি করে, এর গঠনের অন্তর্নিহিত তথ্য ব্যবহার করে স্ট্রংলি কানেক্টেড কম্পোনেন্ট (এসসিসি) ডিটারমিন করে, রানটাইম $O(n+m)$।
...
চলো DFS কলের ক্রম দ্বারা উদ্ভূত ট্রি কনসিডার করি, যেটিকে আমরা **DFS ট্রি** বলব।
...
আমরা ডিটারমিন করেছি যে একটি এসসিসি-র সব ভার্টেক্সের একটি কমন অ্যানসেস্টর আছে, DFS কল দ্বারা প্রথম ভিজিট করা ভার্টেক্স।
...
{{< /details >}}
লক্ষ্য কর যে এসসিসিগুলো DFS ট্রিকে কানেক্টেড সাবগ্রাফে নিখুঁতভাবে বিভক্ত করে।

অ্যালগরিদমের ধারণা এরকম:

- আমরা DFS কলের একটি ক্রম সম্পাদন করি, অ্যাডজেসেন্সি লিস্টের ভার্টেক্সগুলোতে রিকার্সিভভাবে প্রয়োগ করি।

- একটি ভার্টেক্সের অ্যাডজেসেন্সি লিস্ট ট্রাভার্স শেষ হলে, আমরা কোনোভাবে ডিটারমিন করতে পারি এটি রুট কি না।
...
চলো এখন এই দাবি করার প্রক্রিয়া প্রবর্তন করলে DFS-এর বৈশিষ্ট্যগুলো বিশ্লেষণ করি।
...
- ট্রি-এজ: যদি এটি ট্রি-এজ হয়, এটি প্রথমবার আমরা ভার্টেক্স $u$ খুঁজে পাচ্ছি। তার মানে আমাদের প্রথমে $u$-তে রিকার্সিভভাবে DFS কল করতে হবে এবং তার DFS কল শেষ হওয়ার পরে কনসিডার করতে হবে। যদি ভার্টেক্স $u$ অদাবিকৃত থাকে, তার রুট হয় $v$ অথবা $v$-এর কোনো অ্যানসেস্টর, তাই তারা একই এসসিসি-তে থাকতে হবে।
...
{{< /details >}}
লক্ষ্য কর, যখন দুটি ভার্টেক্স একই কম্পোনেন্টে থাকে, তাদের রুট অবশ্যই উভয় ভার্টেক্সের কমন অ্যানসেস্টর হতে হবে।

{{< hint info >}}
**থিওরেম**


ধরি $v$ একটি ভার্টেক্স। নিচের স্টেটমেন্টগুলো সমতুল্য:
...
{{< /details >}}
এখন, আমাদের সেই পদ্ধতি খুঁজতে হবে যেটি ডিটারমিন করবে একটি ভার্টেক্স রুট কি না, এবং দাবি করার প্রক্রিয়ার বৈশিষ্ট্যগুলো এর সঠিকতার জন্য প্রয়োজনীয়।
...
ধরি $v$ একটি ভার্টেক্স এবং তার সাবট্রি কনসিডার করি।
...
দাবি করার প্রক্রিয়া কনসিডার করলে, $v$-এর সাবট্রি-র বাইরে সব অদাবিকৃত ভার্টেক্সের $t_{in}$ মান $t_{in}[v]$ থেকে ছোট।
এখন আমরা দেখতে পাচ্ছি কীভাবে $t_{in}$ ব্যবহার করে রুট ডিটারমিন করা যায়।
আমরা পৌঁছাতে পারা অদাবিকৃত ভার্টেক্সগুলোর $t_{in}$-এর সর্বনিম্ন মান কনসিডার করি এবং ট্রি-এজের মাধ্যমে এই তথ্য অ্যানসেস্টরদের কাছে প্রচার করি।
প্রচারিত মানকে আমরা $t_{low}$ বলব।

আরো ফর্মালি, আমরা $t_{low}[v]$-কে সংজ্ঞায়িত করি $v$-এর সাবট্রি-র কোনো ভার্টেক্স থেকে সরাসরি এজের মাধ্যমে পৌঁছানো যায় এমন সর্বনিম্ন $t_{in}$ মান হিসেবে।
...
প্রথম নীতি থেকে ডেটা স্ট্রাকচার ডিটারমিন করতে, চলো এটি যে মেথডগুলো ইমপ্লিমেন্ট করতে হবে তা দেখি, যেগুলো শুধু দুটি:
...
স্ট্যাক কনসিডার করলে, এর কমপ্লেক্সিটি $O(n)$-এ অ্যামর্টাইজ হয় কারণ প্রতিটি নোড শুধু একবার পুশ এবং পপ করা হয়।
