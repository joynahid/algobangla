...
বাম ছবিতে তুমি একটি ওয়েটেড আনডিরেক্টেড গ্রাফ দেখতে পাচ্ছ, এবং ডান ছবিতে সংশ্লিষ্ট মিনিমাম স্প্যানিং ট্রি দেখতে পাচ্ছ।

![Random graph](/images/graph/MST_before.png) ![MST of this graph](/images/graph/MST_after.png)

এই আর্টিকেলে মিনিমাম স্প্যানিং ট্রি সম্পর্কিত কিছু গুরুত্বপূর্ণ তথ্য আলোচনা করা হবে, এবং তারপর মিনিমাম স্প্যানিং ট্রি খুঁজে বের করার জন্য ক্রুস্কালের অ্যালগরিদমের সবচেয়ে সরল ইমপ্লিমেন্টেশন দেওয়া হবে।
...
* মিনিমাম স্প্যানিং ট্রি হলো সেই ট্রিও যেটিতে এজের ওয়েটের গুণফল ন্যূনতম।
  (সকল এজের ওয়েটকে তাদের লগারিদম দিয়ে প্রতিস্থাপন করে এটি সহজেই প্রুফ করা যায়)
...
নিচের কোডটি উপরে দেখানো অ্যালগরিদমটি সরাসরি ইমপ্লিমেন্ট করে, এবং এর টাইম কমপ্লেক্সিটি $O(M \log M + N^2)$।
...
সবশেষে, দুটি ট্রির ইউনিয়ন `tree_id[]` অ্যারের মধ্য দিয়ে একটি সাধারণ পাস করে $O(N)$-এ কমপ্লিট হয়।
...
যদি মূল গ্রাফটি কানেক্টেড হয়, তাহলে ফলাফল গ্রাফটিও কানেক্টেড হবে।
কারণ অন্যথায় দুটি কম্পোনেন্ট থাকত যেগুলো অন্তত একটি এজ দিয়ে কানেক্ট করা যেত। কিন্তু এটি অসম্ভব, কারণ ক্রুস্কাল সেই এজগুলোর একটি নির্বাচন করত, যেহেতু কম্পোনেন্টগুলোর আইডি ভিন্ন।
...
আমরা ইন্ডাকশন ব্যবহার করে প্রুফ করতে পারি যে "যদি $F$ হলো অ্যালগরিদমের যেকোনো পর্যায়ে সিলেক্টেড এজের সেট, তাহলে একটি এমএসটি আছে যেটি $F$-এর সকল এজ ধারণ করে"।
...
লক্ষ্য কর যে $f$-এর ওয়েট $e$-এর ওয়েটের চেয়ে ছোট হতে পারে না, কারণ তাহলে ক্রুস্কাল আগেই $f$ নির্বাচন করত।
...
এটি প্রস্তাবনাটি প্রুফ করে।
যার অর্থ হলো সকল এজ ইটারেট করার পর ফলাফল এজ সেট কানেক্টেড হবে, এবং একটি এমএসটি-তে ধারণকৃত হবে, যার মানে এটি ইতিমধ্যেই একটি এমএসটি।

## উন্নত ইমপ্লিমেন্টেশন

আমরা [**ডিসজয়েন্ট সেট ইউনিয়ন** (ডিএসইউ)](../data_structures/disjoint_set_union.md) ডেটা স্ট্রাকচার ব্যবহার করে ক্রুস্কালের অ্যালগরিদমের একটি দ্রুততর ইমপ্লিমেন্টেশন লিখতে পারি যার টাইম কমপ্লেক্সিটি প্রায় $O(M \log N)$। [এই আর্টিকেলে](mst_kruskal_with_dsu.md) এই পদ্ধতিটি বিস্তারিত বর্ণনা করা হয়েছে।
