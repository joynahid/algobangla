...
ট্রিপ হলো একটি ডাটা স্ট্রাকচার যা বাইনারি ট্রি এবং বাইনারি হিপ একত্রিত করে (তাই নাম: tree + heap $\Rightarrow$ Treap)।

আরো নির্দিষ্টভাবে বলতে গেলে, ট্রিপ এমন একটি ডাটা স্ট্রাকচার যা $(X, Y)$ জোড়া একটি বাইনারি ট্রিতে এমনভাবে সংরক্ষণ করে যে এটা$X$ অনুসারে একটি বাইনারি সার্চ ট্রি এবং $Y$ অনুসারে একটি বাইনারি হিপ।
...
ট্রিপকে প্রায়ই "কার্তেসিয়ান ট্রি"-ও বলা হয়, কারণ এটাসহজেই কার্তেসিয়ান সমতলে স্থাপন করা যায়:
...
এই ইমপ্লিমেন্টেশনে, $X$ মানগুলো হলো কী (এবং একই সাথে ট্রিপে স্টোর করা মান), এবং $Y$ মানগুলোকে **প্রায়োরিটি** বলা হয়। প্রায়োরিটি ছাড়া, ট্রিপটি $X$ অনুসারে একটি সাধারণ বাইনারি সার্চ ট্রি হতো, এবং একই $X$ মানের সেট অনেক ভিন্ন ট্রির সাথে সংশ্লিষ্ট হতে পারত, কিছু অধঃপতিত (উদাহরণস্বরূপ, লিঙ্কড লিস্টের আকারে), এবং তাই অত্যন্ত ধীর (প্রধান অপারেশনগুলোর কমপ্লেক্সিটি $O(N)$ হতো)।

একই সাথে, **প্রায়োরিটি** (যখন তারা অনন্য) ট্রিটিকে **অনন্যভাবে** নির্দিষ্ট করতে দেয় যা তৈরি হবে (অবশ্যই, এটামান যোগ করার ক্রমের উপর নির্ভর করে না), যা সংশ্লিষ্ট থিওরেম দিয়ে প্রুফ করা যায়। স্পষ্টতই, যদি তুমি **এলোমেলোভাবে প্রায়োরিটি বেছে নেন**, তুমি গড়ে অধঃপতিত-নয় এমন ট্রি পাবেন, যা প্রধান অপারেশনগুলোর জন্য $O(\log N)$ কমপ্লেক্সিটি নিশ্চিত করবে। তাই এই ডাটা স্ট্রাকচারের আরেকটি নাম - **র‍্যান্ডমাইজড বাইনারি সার্চ ট্রি**।

## অপারেশন

একটি ট্রিপ নিচের অপারেশন দেয়:
...
- **Build ($X_1$, ..., $X_N$)** $O(N)$-এ।
  মানের তালিকা থেকে একটি ট্রি তৈরি করে। এটালিনিয়ার সময়ে করা যায় (ধরে নিচ্ছি $X_1, ..., X_N$ সাজানো আছে)।
...
এছাড়াও, ট্রিপ একটি বাইনারি সার্চ ট্রি হওয়ায়, এটাঅন্যান্য অপারেশনও ইমপ্লিমেন্ট করতে পারে, যেমন $K$-তম বৃহত্তম উপাদান খোঁজা বা কোনো উপাদানের ইনডেক্স বের করা।
...
**Split ($T$, $X$)** ট্রি $T$ কে ২টি সাবট্রি $L$ ও $R$ এ আলাদা করে (যেগুলো স্প্লিটের রিটার্ন ভ্যালু) যেন $L$ তে কী $X_L \le X$ এর সব উপাদান থাকে, এবং $R$ তে কী $X_R > X$ এর সব উপাদান থাকে। এই অপারেশনের কমপ্লেক্সিটি $O (\log N)$ এবং এটাপরিষ্কার রিকার্শন দিয়ে ইমপ্লিমেন্ট করা হয়:
...
বিকল্পভাবে, ইনসার্ট প্রাথমিক ট্রিপকে $X$ এ স্প্লিট করে এবং নতুন নোডের সাথে ২টি মার্জ করেও করা যায় (ছবি দেখো)।
...
বিকল্পভাবে, আমরা ২টি স্প্লিট অপারেশন দিয়ে $X$ ধারণকারী সাবট্রি আলাদা করে বাকি ট্রিপগুলো মার্জ করতে পারি (ছবি দেখো)।
...
এটাআমাদের আইটেম সংজ্ঞা। লক্ষ্য করো দুটি সন্তান পয়েন্টার আছে, এবং একটি পূর্ণসংখ্যা কী (BST এর জন্য) ও একটি পূর্ণসংখ্যা প্রায়োরিটি (হিপের জন্য)। প্রায়োরিটি একটি র‍্যান্ডম নম্বর জেনারেটর ব্যবহার করে ডিটারমিন করা হয়।
...
`t` হলো স্প্লিট করার ট্রিপ, এবং `key` হলো BST মান যা দিয়ে স্প্লিট করতে হবে। লক্ষ্য করো আমরা কোথাও ফলাফল `return` করি না, বরং এভাবে ব্যবহার করি:
...
এই `split` ফাংশনটি বুঝতে কঠিন হতে পারে, কারণ এতে পয়েন্টার (`pitem`) এবং সেই পয়েন্টারের রেফারেন্স (`pitem &l`) উভয়ই আছে। চলো `split(t, k, l, r)` ফাংশন কলের অর্থ বুঝি: "ট্রিপ `t` কে মান `k` দিয়ে দুটি ট্রিপে স্প্লিট করো, এবং বাম ট্রিপ `l` এ ও ডান ট্রিপ `r` এ সংরক্ষণ করো"। এখন, চলো এই সংজ্ঞা দুটি রিকার্সিভ কলে প্রয়োগ করি, আগের বিভাগে বিশ্লেষিত কেস ওয়ার্ক ব্যবহার করে: (প্রথম if শর্তটি খালি ট্রিপের জন্য একটি তুচ্ছ ভিত্তি ক্ষেত্র)

১. যখন রুট নোডের মান $\le$ key, আমরা `split (t->r, key, t->r, r)` কল করি, যার অর্থ: "ট্রিপ `t->r` (`t` এর ডান সাবট্রি) কে মান `key` দিয়ে স্প্লিট করো এবং বাম সাবট্রি `t->r` এ ও ডান সাবট্রি `r` এ সংরক্ষণ করো"। এরপর, আমরা `l = t` সেট করি। লক্ষ্য করো এখন `l` ফলাফল মানে `t->l`, `t` এবং `t->r` (যা আমাদের রিকার্সিভ কলের ফলাফল) সবই ইতিমধ্যে সঠিক ক্রমে মার্জ হয়ে আছে! তোমার থামা উচিত নিশ্চিত করতে যে `l` ও `r` এর এই ফলাফল আগে ইমপ্লিমেন্টেশন বর্ণনায় আমরা যা আলোচনা করেছি তার সাথে ঠিক মিলে যায়।
২. যখন রুট নোডের মান key এর চেয়ে বড়, আমরা `split (t->l, key, l, t->l)` কল করি, যার অর্থ: "ট্রিপ `t->l` (`t` এর বাম সাবট্রি) কে মান `key` দিয়ে স্প্লিট করো এবং বাম সাবট্রি `l` এ ও ডান সাবট্রি `t->l` এ সংরক্ষণ করো"। এরপর, আমরা `r = t` সেট করি। লক্ষ্য করো এখন `r` ফলাফল মানে `t->l` (যা আমাদের রিকার্সিভ কলের ফলাফল), `t` এবং `t->r`, সবই ইতিমধ্যে সঠিক ক্রমে মার্জ হয়ে আছে! তোমার থামা উচিত নিশ্চিত করতে যে `l` ও `r` এর এই ফলাফল আগে ইমপ্লিমেন্টেশন বর্ণনায় আমরা যা আলোচনা করেছি তার সাথে ঠিক মিলে যায়।

তোমার যদি ইমপ্লিমেন্টেশন বুঝতে এখনো সমস্যা হয়, তোমার এটাকে _আরোহী পদ্ধতিতে_ দেখা উচিত, অর্থাৎ: রিকার্সিভ কলগুলো বারবার ভেঙে দেখার চেষ্টা কোরো *না*। ধরে নাও স্প্লিট ইমপ্লিমেন্টেশন খালি ট্রিপে সঠিকভাবে কাজ করে, তারপর একটি একক নোড ট্রিপে চালানোর চেষ্টা করো, তারপর দুই নোড ট্রিপে, এভাবে এগিয়ে যাও, প্রতিবার তোমার জ্ঞান ব্যবহার করো যে ছোট ট্রিপে স্প্লিট কাজ করে।
...
ট্রিপের কার্যকারিতা বাড়াতে, প্রায়ই প্রতিটি নোডের সাবট্রিতে নোডের সংখ্যা সংরক্ষণ করা প্রয়োজন - `item` স্ট্রাকচারে `int cnt` ফিল্ড। উদাহরণস্বরূপ, এটাট্রি-র $K$-তম বৃহত্তম উপাদান $O (\log N)$-এ খুঁজতে, অথবা সাজানো তালিকায় উপাদানের ইনডেক্স একই কমপ্লেক্সিটিতে বের করতে ব্যবহার করা যায়। এই অপারেশনগুলোর ইমপ্লিমেন্টেশন সাধারণ বাইনারি সার্চ ট্রির মতোই হবে।

যখন ট্রি পরিবর্তন হয় (নোড যোগ বা সরানো হয় ইত্যাদি), কিছু নোডের `cnt` সেই অনুযায়ী আপডেট করা উচিত। আমরা দুটি ফাংশন তৈরি করবো: `cnt()` বর্তমান `cnt` এর মান রিটার্ন করবে অথবা নোড না থাকলে ০, এবং `upd_cnt()` এই নোডের `cnt` এর মান আপডেট করবে ধরে নিয়ে যে এর সন্তান L ও R এর `cnt` মান ইতিমধ্যে আপডেট হয়ে আছে। স্পষ্টতই `cnt` মান আপডেট রাখতে `insert`, `erase`, `split` এবং `merge` এর শেষে `upd_cnt()` কল যোগ করাই যথেষ্ট।
...
দ্রষ্টব্য: `upd_cnt(t)` কল শুধু তখনই প্রয়োজন যখন তোমার সাবট্রির আকার দরকার।

উপরের পদ্ধতি সবসময় একটি পুরোপুরি সুষম ট্রি দেয়, যা সাধারণত ব্যবহারিক উদ্দেশ্যে ভালো, কিন্তু প্রতিটি নোডে শুরুতে নির্ধারিত প্রায়োরিটি স্টোর করা না থাকার মূল্যে। তাই, এই পদ্ধতি নিচের সমস্যা সমাধানে কাজে আসবে না:

{{< hint info >}}
**[acmsguru - Cartesian Tree](https://codeforces.com/problemsets/acmsguru/problem/99999/155)**

$(x_i, y_i)$ জোড়ার একটি ক্রম দেওয়া আছে, এদের উপর একটি কার্তেসিয়ান ট্রি তৈরি করো। সব $x_i$ এবং সব $y_i$ অনন্য।
{{< /hint >}}
লক্ষ্য করো এই সমস্যায় প্রায়োরিটি এলোমেলো নয়, তাই একটি একটি করে শীর্ষ ইনসার্ট করলে কোয়াড্রাটিক সমাধান হতে পারে।
...
ইমপ্লিসিট ট্রিপ হলো সাধারণ ট্রিপের একটি সরল পরিবর্তন যা একটি অত্যন্ত শক্তিশালী ডাটা স্ট্রাকচার। আসলে, ইমপ্লিসিট ট্রিপকে নিচের প্রক্রিয়াগুলো ইমপ্লিমেন্ট করা একটি অ্যারে হিসেবে কনসিডার করা যায় (সবই অনলাইন মোডে $O (\log N)$-এ):
...
লক্ষ্য করো একটি নোডের কী হলো তার চেয়ে ছোট নোডের সংখ্যা (এরকম নোড শুধু এর বাম সাবট্রিতেই নয়, এর পূর্বপুরুষদের বাম সাবট্রিতেও থাকতে পারে)।
...
- **উপাদান মুছে ফেলা।**
  এই অপারেশন আরো সহজ: মুছতে হবে এমন উপাদান $T$ খুঁজুন, এর সন্তান $L$ ও $R$ এর মার্জ করো, এবং মার্জের ফলাফল দিয়ে উপাদান $T$ প্রতিস্থাপন করো। আসলে, ইমপ্লিসিট ট্রিপে উপাদান মুছে ফেলা সাধারণ ট্রিপের মতোই।
- ব্যবধানে **যোগফল / সর্বনিম্ন** ইত্যাদি খোঁজা।
  প্রথমে, `item` স্ট্রাকচারে একটি অতিরিক্ত ফিল্ড $F$ তৈরি করো এই নোডের সাবট্রির জন্য লক্ষ্য ফাংশনের মান সংরক্ষণ করতে। সাবট্রির আকার বজায় রাখার মতোই এই ফিল্ড বজায় রাখা সহজ: একটি ফাংশন তৈরি করো যা সন্তানদের মানের উপর ভিত্তি করে একটি নোডের এই মান গণনা করে এবং ট্রি পরিবর্তনকারী সব ফাংশনের শেষে এই ফাংশনের কল যোগ করো।
  দ্বিতীয়ত, যেকোনো ব্যবধান $[A; B]$ এর জন্য একটি কুয়েরি প্রক্রিয়া করতে হবে।
  ট্রির যে অংশ $[A; B]$ ব্যবধানের সাথে সংশ্লিষ্ট তা পেতে, আমাদের $split(T, T_2, T_3, B+1)$, তারপর $split(T_2, T_1, T_2, A)$ কল করতে হবে: এরপর $T_2$ $[A; B]$ ব্যবধানের সব উপাদান নিয়ে গঠিত হবে, এবং শুধু সেগুলোই। তাই কুয়েরির উত্তর $T_2$ এর রুটের $F$ ফিল্ডে স্টোর করা থাকবে। কুয়েরির উত্তর দেওয়ার পর, $merge(T, T_1, T_2)$ এবং $merge(T, T, T_3)$ কল করে ট্রি রিকভার করতে হবে।
- ব্যবধানে **যোগ / রং করা।**
...
- ব্যবধানে **উল্টানো।**
  এটিও আগের অপারেশনের মতো: আমাদের একটি বুলিয়ান ফ্ল্যাগ `rev` যোগ করতে হবে এবং বর্তমান নোডের সাবট্রি উল্টাতে হলে এটাtrue সেট করতে হবে। এই মান "পুশ" করা একটু জটিল - আমরা এই নোডের সন্তানদের অদলবদল করি এবং তাদের জন্য এই ফ্ল্যাগ true সেট করি।
