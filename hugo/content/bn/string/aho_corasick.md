...
অ্যালগরিদমটি $O(m k)$ সময়ে একটি ট্রাই-ভিত্তিক ফিনাইট স্টেট অটোমেটন তৈরি করে এবং তারপর টেক্সট প্রক্রিয়া করতে এটা ব্যবহার করে।
...
সোজা কথায়, একটি ট্রাই হলো একটি রুটেড ট্রি, যেখানে ট্রির প্রতিটি এজ কোনো অক্ষর দিয়ে লেবেলযুক্ত এবং একটি ভার্টেক্স থেকে বের হওয়া এজগুলোর লেবেল ভিন্ন।
...
তারমানে, স্ট্রিং-এর একটি সেটের জন্য একটি ট্রাই হলো এমন ট্রাই যেন প্রতিটি $\text{output}$ ভার্টেক্স সেটের একটি স্ট্রিং-এর সাথে এবং বিপরীতভাবে সেটের প্রতিটি স্ট্রিং একটি $\text{output}$ ভার্টেক্সের সাথে সম্পর্কিত।

আমরা এখন বর্ণনা করি কিভাবে দেওয়া স্ট্রিং-এর সেটের জন্য তাদের মোট দৈর্ঘ্যের সাপেক্ষে লিনিয়ার সময়ে একটি ট্রাই তৈরি করতে হয়।
...
শুরুতে, ট্রাই শুধু একটি ভার্টেক্স নিয়ে গঠিত - রুট - যার ইনডেক্স $0$।
...
এই ইমপ্লিমেন্টেশন স্পষ্টতই লিনিয়ার সময়ে চলে, এবং যেহেতু প্রতিটি ভার্টেক্স $k$ লিংক সংরক্ষণ করে, এটা $O(m k)$ মেমোরি ব্যবহার করবে।
...
ধরো আমরা দেওয়া স্ট্রিং-এর সেটের জন্য একটি ট্রাই তৈরি করেছি।
এখন এটাকে ভিন্ন দৃষ্টিকোণ থেকে দেখি।
...
সোজা কথায়, ধরো আমরা একটি স্ট্রিং $t$-এর সাথে সম্পর্কিত স্টেটে আছি, এবং আমরা $c$ অক্ষর ব্যবহার করে ভিন্ন স্টেটে ট্রানজিশন করতে চাই।
যদি এই অক্ষর $c$ দিয়ে লেবেলযুক্ত এজ থাকে, তাহলে আমরা সরাসরি এই এজ ধরে $t + c$-এর সাথে সম্পর্কিত ভার্টেক্সে যেতে পারি।
যদি এরকম কোনো এজ না থাকে, যেহেতু আমরা এই ইনভ্যারিয়েন্ট বজায় রাখতে চাই যে বর্তমান স্টেট হলো প্রক্রিয়াকৃত স্ট্রিং-এ দীর্ঘতম আংশিক মিল, আমাদের ট্রাইয়ে স্ট্রিং $t$-এর প্রকৃত সাফিক্স যেটা দীর্ঘতম সেটা খুঁজতে হবে, এবং সেখান থেকে ট্রানজিশনের চেষ্টা করতে হবে।

উদাহরণস্বরূপ, ট্রাই $ab$ এবং $bc$ স্ট্রিং দিয়ে তৈরি, এবং আমরা বর্তমানে $ab$-এর সাথে সম্পর্কিত ভার্টেক্সে আছি, যেটা একটি $\text{output}$ ভার্টেক্সও।
...
একটি ভার্টেক্স $p$-এর জন্য **সাফিক্স লিংক** হলো একটি এজ যা ভার্টেক্স $p$-এর সাথে সম্পর্কিত স্ট্রিং-এর দীর্ঘতম প্রকৃত সাফিক্সের দিকে পয়েন্ট করে।
একমাত্র বিশেষ ক্ষেত্র হলো ট্রাইয়ের রুট, যার সাফিক্স লিংক নিজের দিকেই পয়েন্ট করবে।
এখন আমরা অটোমেটনের ট্রানজিশন সম্পর্কিত স্টেটমেন্টটি এভাবে পুনর্গঠন করতে পারি:
...
রুট ভার্টেক্স এবং এর সকল তাৎক্ষণিক চিলড্রেনের সাফিক্স লিংক রুট ভার্টেক্সের দিকে পয়েন্ট করে।
ট্রিতে আরও গভীরে যেকোনো ভার্টেক্স $v$-এর জন্য, সাফিক্স লিংক এভাবে গণনা করা যায়:
যদি $p$ হলো $v$-এর পূর্বসূরি এবং $c$ হলো $p$ থেকে $v$-তে যাওয়ার এজের অক্ষর,
$p$-তে যাও,
তারপর এর সাফিক্স লিংক ফলো করো, এবং সেখান থেকে $c$ অক্ষর দিয়ে ট্রানজিশন করো।
...
চলো ইমপ্লিমেন্টেশনে যাই।
লক্ষ্য করো এখন আমরা প্রতিটি ভার্টেক্স $v$-এর জন্য পূর্বসূরি $p$ এবং $p$ থেকে $v$-তে এজের অক্ষর $pch$ সংরক্ষণ করব।
এছাড়া প্রতিটি ভার্টেক্সে সাফিক্স লিংক $\text{link}$ (বা $-1$ যদি এখনো গণনা না হয়ে থাকে), এবং $\text{go}[k]$ অ্যারেতে (array) প্রতিটি প্রতীকের জন্য মেশিনে ট্রানজিশন (আবার $-1$ যদি এখনো গণনা না হয়ে থাকে) সংরক্ষণ করব।
...
ধারণাটির চিত্রণের জন্য [স্ট্যানফোর্ড স্লাইডের](http://web.stanford.edu/class/archive/cs/cs166/cs166.1166/lectures/02/Slides02.pdf) ১০৩ নম্বর স্লাইড দেখো।
...
এই পদ্ধতির আগে যেটা দেখানো হয়েছে তার তুলনায় কিছু সুবিধা আছে কারণ, মোট দৈর্ঘ্য $m$-এর পরিবর্তে, এর রানিং টাইম শুধু ট্রাইয়ের ভার্টেক্স সংখ্যা $n$-এর উপর নির্ভর করে। তাছাড়া, একটি পার্সিস্টেন্ট অ্যারে ডেটা স্ট্রাকচার ব্যবহার করে বড় বর্ণমালার জন্য এটা অভিযোজিত করা সম্ভব, ফলে নির্মাণ সময় $O(mk)$-এর পরিবর্তে $O(n \log k)$ হয়, যা উল্লেখযোগ্য উন্নতি কারণ $m$, $n^2$ পর্যন্ত হতে পারে।
...
ধরো এই মুহূর্তে আমরা একটি ভার্টেক্স $v$-তে দাঁড়িয়ে আছি এবং একটি অক্ষর $c$ কনসিডার করছি। মূলত আমাদের দুটি ক্ষেত্র আছে:
...
এভাবে, আমরা প্রতিটি ভার্টেক্স-অক্ষর জোড়ায় $O(1)$ সময় ব্যয় করি, ফলে রানিং টাইম $O(nk)$ হয়। এখানে প্রধান ওভারহেড হলো প্রথম ক্ষেত্রে $u$ থেকে অনেক ট্রানজিশন কপি করা, যখন দ্বিতীয় ক্ষেত্রের ট্রানজিশনগুলো ট্রাই গঠন করে এবং সকল ভার্টেক্সে মিলিয়ে $n$ হয়। $go[u][c]$ কপি এড়াতে, আমরা একটি পার্সিস্টেন্ট অ্যারে ডেটা স্ট্রাকচার ব্যবহার করতে পারি, যেটা দিয়ে আমরা প্রথমে $go[u]$-কে $go[v]$-তে কপি করি এবং তারপর শুধু সেই অক্ষরগুলোর জন্য মান আপডেট করি যেখানে ট্রানজিশন ভিন্ন হবে। এটা $O(n \log k)$ অ্যালগরিদম দেয়।

## অ্যাপ্লিকেশন

### একটি টেক্সটে দেওয়া সেটের সকল স্ট্রিং খোঁজা

আমাদের একটি স্ট্রিং-এর সেট এবং একটি টেক্সট দেওয়া আছে।
আমাদের $O(\text{len} + \text{ans})$ সময়ে দেওয়া টেক্সটে সেটের সকল স্ট্রিং-এর সকল উপস্থিতি প্রিন্ট করতে হবে, যেখানে $\text{len}$ হলো টেক্সটের দৈর্ঘ্য এবং $\text{ans}$ হলো উত্তরের আকার।
...
তবে এটা কোনোমতেই মিল অর্জনের একমাত্র সম্ভাব্য ক্ষেত্র নয়:
...
এটা সবচেয়ে দক্ষ সমাধান নয়, কারণ এতে সামগ্রিকভাবে $O(n ~ \text{len})$ কমপ্লেক্সিটি হয়।
তবে, সাফিক্স লিংক দিয়ে পৌঁছানো নিকটতম $\text{output}$ ভার্টেক্স গণনা ও সংরক্ষণ করে এটা অপটিমাইজ করা যায় (এটাকে কখনো কখনো **এক্সিট লিংক** বলা হয়)।
...
তুমি যদি শুধু উপস্থিতির সংখ্যা গণনা করতে চাও এবং ইনডেক্স নিজে না চাও, তাহলে প্রতিটি ভার্টেক্স $v$-এর জন্য সাফিক্স লিংক পথে চিহ্নিত ভার্টেক্সের সংখ্যা গণনা করতে পারো।
এটা মোট $O(n)$ সময়ে গণনা করা যায়।
তাই সকল মিল $O(\text{len})$-এ যোগ করা যায়।

### দেওয়া দৈর্ঘ্যের লেক্সিকোগ্রাফিক্যালি ক্ষুদ্রতম স্ট্রিং খোঁজা যা কোনো দেওয়া স্ট্রিং-এর সাথে মেলে না
...
মনে রাখো $\text{output}$ ভার্টেক্সগুলো হলো সেই স্টেট যেখানে সেটের কোনো স্ট্রিং-এর সাথে মিল আছে।
...
এই কাজটি $O(L)$-এ সমাধান করা যায় উদাহরণস্বরূপ [DFS](../graph/depth-first-search.md) দিয়ে।

### সকল দেওয়া স্ট্রিং ধারণকারী সংক্ষিপ্ততম স্ট্রিং খোঁজা

এখানে আমরা একই ধারণা ব্যবহার করি।
প্রতিটি ভার্টেক্সের জন্য আমরা একটি মাস্ক সংরক্ষণ করি যেটা বোঝায় এই স্টেটে কোন স্ট্রিংগুলো মেলে।
তাহলে সমস্যাটি এভাবে পুনর্গঠন করা যায়:
শুরুতে $(v = \text{root},~ \text{mask} = 0)$ স্টেটে থেকে, আমরা $(v,~ \text{mask} = 2^n - 1)$ স্টেটে পৌঁছাতে চাই, যেখানে $n$ হলো সেটে স্ট্রিং-এর সংখ্যা।
যখন আমরা একটি অক্ষর ব্যবহার করে এক স্টেট থেকে অন্য স্টেটে ট্রানজিশন করি, আমরা সেই অনুযায়ী মাস্ক আপডেট করি।
[BFS](../graph/breadth-first-search.md) চালিয়ে আমরা ক্ষুদ্রতম দৈর্ঘ্যের $(v,~ \text{mask} = 2^n - 1)$ স্টেটে পৌঁছানোর পথ খুঁজতে পারি।
...
আগের সমস্যার মতো, আমরা প্রতিটি ভার্টেক্সের জন্য এর সাথে সম্পর্কিত মিলের সংখ্যা গণনা করি (অর্থাৎ সাফিক্স লিংক ব্যবহার করে পৌঁছানো চিহ্নিত ভার্টেক্সের সংখ্যা)।
...
তাই আমরা DFS দিয়ে এরকম পথ খুঁজতে পারি (এবং যদি সার্চ এজগুলো তাদের স্বাভাবিক ক্রমে দেখে, তাহলে পাওয়া পথ স্বয়ংক্রিয়ভাবে লেক্সিকোগ্রাফিক্যালি ক্ষুদ্রতম হবে)।
