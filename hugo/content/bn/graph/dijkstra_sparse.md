---
title: "স্পার্স গ্রাফে ডায়াক্সট্রা"
tags: 
weight: 20
---
# স্পার্স গ্রাফে ডায়াক্সট্রা

সমস্যার স্টেটমেন্ট, অ্যালগরিদম সহ ইমপ্লিমেন্টেশন এবং প্রুফ [ডায়াক্সট্রা অ্যালগরিদম](dijkstra.md) আর্টিকেলে পাওয়া যাবে।

## অ্যালগরিদম

আমরা জানি যে ডায়াক্সট্রা অ্যালগরিদমের কমপ্লেক্সিটি নির্ণয়ে আমরা দুটি ফ্যাক্টর ব্যবহার করেছিলাম:
...
এটা স্পষ্ট যে এই কমপ্লেক্সিটি ডেন্স গ্রাফের জন্য অপটিমাল, অর্থাৎ যখন $m \approx n^2$।
...
এটা কমপ্লিট করতে আমরা একাধিক সহায়ক ডেটা স্ট্রাকচারের একটি ভ্যারিয়েশন ব্যবহার করতে পারি।
সবচেয়ে দক্ষ হলো **ফিবোনাচি হিপ**, যেটা প্রথম অপারেশন $O(\log n)$ এ এবং দ্বিতীয় অপারেশন $O(1)$ এ চালাতে দেয়।
অতএব আমরা ডায়াক্সট্রা অ্যালগরিদমের জন্য $O(n \log n + m)$ কমপ্লেক্সিটি পাব, যেটা শর্টেস্ট পাথ সার্চ সমস্যার তাত্ত্বিক ন্যূনতমও।
অতএব এই অ্যালগরিদম অপটিমালভাবে কাজ করে, এবং ফিবোনাচি হিপ হলো অপটিমাল ডেটা স্ট্রাকচার।
কোনো ডেটা স্ট্রাকচার নেই যেটা উভয় অপারেশন $O(1)$ এ সম্পাদন করতে পারে, কারণ এটা র‍্যান্ডম সংখ্যার একটি তালিকা লিনিয়ার সময়ে সর্ট করার সুযোগ দিত, যা অসম্ভব।
মজার বিষয় হলো, থরাপের একটি অ্যালগরিদম আছে যেটা $O(m)$ সময়ে শর্টেস্ট পাথ খুঁজে পায়, তবে শুধুমাত্র ইন্টিজার ওয়েটের জন্য কাজ করে, এবং সম্পূর্ণ ভিন্ন ধারণা ব্যবহার করে।
তাই এটা কোনো বৈপরীত্য তৈরি করে না।
ফিবোনাচি হিপ এই কাজের জন্য অপটিমাল কমপ্লেক্সিটি দেয়।
তবে এগুলো ইমপ্লিমেন্ট করা বেশ জটিল, এবং এগুলোর হিডেন কনস্ট্যান্টও বেশ বড়।

একটি সমঝোতা হিসেবে তুমি এমন ডেটা স্ট্রাকচার ব্যবহার করতে পারো, যেগুলো উভয় ধরনের অপারেশন (মিনিমাম বের করা এবং একটি আইটেম আপডেট করা) $O(\log n)$ এ করে।
তখন ডায়াক্সট্রা অ্যালগরিদমের কমপ্লেক্সিটি $O(n \log n + m \log n) = O(m \log n)$।

C++ এমন দুটি ডেটা স্ট্রাকচার দেয়: `set` এবং `priority_queue`।
...
এটা একটি উপাদান মুছে ফেলার অপারেশন সমর্থন করে না।
এই কারণে আমাদের একটি "ওয়ার্কঅ্যারাউন্ড" করতে হয়, যেটা আসলে $\log n$ এর পরিবর্তে সামান্য খারাপ $\log m$ ফ্যাক্টরে নিয়ে যায় (যদিও কমপ্লেক্সিটির দিক থেকে এগুলো অভিন্ন)।

## ইমপ্লিমেন্টেশন

### set

চলো `set` কন্টেইনার দিয়ে শুরু করি।
...
আমরা সেই তথ্য সংরক্ষণ করতে `set` ব্যবহার করব, এবং এটা দিয়ে সর্বনিম্ন দূরত্বের ভার্টেক্সও খুঁজব।
এটা একরকম কিউ-র মতো কাজ করে।
...
প্রতিটি ইটারেশনের শুরুতে, পরবর্তী পেয়ার বের করার পর, আমরা পরীক্ষা করি এটা একটি গুরুত্বপূর্ণ পেয়ার নাকি ইতিমধ্যে পুরনো এবং প্রক্রিয়াকৃত পেয়ার।
...
এটাকে ঊর্ধ্বক্রমে সাজাতে, আমরা হয় এতে নেগেটেড দূরত্ব সংরক্ষণ করতে পারি, অথবা একটি ভিন্ন সর্টিং ফাংশন পাস করতে পারি।
...
তুমি পারফরম্যান্স আরেকটু উন্নত করতে পারো যদি কন্টেইনারে পেয়ার না রেখে শুধু ভার্টেক্স ইনডেক্স রাখো।
এক্ষেত্রে আমাদের কম্পারিসন অপারেটর ওভারলোড করতে হবে:
এটাকে $d[]$ এ স্টোর করা দূরত্ব ব্যবহার করে দুটি ভার্টেক্সের তুলনা করতে হবে।
...
বাস্তবে এটা পারফরম্যান্স উল্লেখযোগ্যভাবে বাড়ায়, বিশেষত যখন বড় ডেটা টাইপ ব্যবহার করা হয় দূরত্ব সংরক্ষণে, যেমন `long long` বা `double`।
